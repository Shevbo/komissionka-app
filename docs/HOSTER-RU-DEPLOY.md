...
**Репозиторий, код, документация, скрипты — на сервере.** VPS Hoster.ru (83.69.248.175), каталог `~/komissionka`. Локальная разработка заморожена. Все изменения, тестирование, БД, API — на сервере. Локальный `npm run dev` и локальная БД не используются.
...
Добавьте в `~/.ssh/config` (Windows: `C:\Users\ВАШ_ПОЛЬЗОВАТЕЛЬ\.ssh\config`):
...
2. Заполните (подставьте свои данные):
   ...
   Или выполните с локального ПК: `.\scripts\fix-server-images-hoster.ps1` — добавит UPLOADS_AGENT_DIR и перезапустит.
...
Или если агент работает с другого хоста (например, с локальной машины разработчика против прод-сервера):
...
- **Миграции:** локально — `npx prisma migrate dev`, на сервере — только `npx prisma migrate deploy`.
