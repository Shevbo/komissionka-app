...
- В корне репозитория есть общий лог `what's new.md`. В каждом модуле (app, agent, telegram-bot) должен вестись свой модульный лог `what's new.md` в корне модуля.
- При ЛЮБОМ изменении кода или конфигурации обязательно:
  ...
  2) Добавить запись в корневой `what's new.md` в раздел **Записи** в виде:
  ...
  3) Обновить модульный лог соответствующего компонента (`agent/what's new.md`, `telegram-bot/what's new.md`, и т.д.) с датой, версией и кратким описанием изменений.
...
- Не допускать ситуаций, когда версия в `version.json` изменена, а соответствующих записей в `what's new.md` (корневом и модульном) нет.`;
  }
