...
- НЕ используй sed/awk/cat/head/tail через run_command, чтобы «посмотреть кусок файла» — эти команды не в белом списке и будут блокироваться.
- Для больших файлов вместо этого используй read_file с параметрами offset_lines и limit_lines, чтобы прочитать только нужный диапазон строк и экономить токены. Сначала найди место через grep, затем дочитай нужный фрагмент read_file(offset_lines=..., limit_lines=...).
...
- В корне репозитория всегда обновляй общий файл whats new.md и модульный лог компонента (например agent/whats new.md) при любом изменении кода.
- Перед изменениями увеличивай версию в version.json по правилам из docs/VERSIONING-RULES.md и записывай в лог: дату, краткий текст запроса от пользователя, какие файлы и модули изменены, а также пары версий «было → стало» для app, agent и tgbot.
- Не меняй версии без соответствующих записей в логах и не оставляй описания типа «мелкие правки» — формулируй понятно, что именно было сделано.`;
  }
