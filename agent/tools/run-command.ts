...
const ALLOWED_COMMANDS: ReadonlyArray<RegExp | string> = [
  ...
  // Разрешаем аргументы (например scripts/delete-item.ts <id>). Инъекции блокируются DANGEROUS_CHARS / isDangerousCommand.
  /^npx\s+tsx\s+scripts\/[a-zA-Z0-9_-]+\.ts(\s+[\s\S]+)?$/,
];
...
npx tsx scripts/<имя>.ts [args...] — запуск скриптов из scripts/ (например seed-five-users.ts, seed-demo.ts); имя файла — только латиница, цифры, дефис, подчёркивание
